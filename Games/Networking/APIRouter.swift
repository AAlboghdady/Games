...
//    static func makeRequest<S: Codable>(session: URLSession, request: URLRequest, model: S.Type, onCompletion: @escaping(Result<S, NetworkError>) -> ()) -> URLSessionTask
//    static func makeGetRequest<T: Codable> (path: EndPoint, queries: Parameters, onCompletion: @escaping(Result<T, NetworkError>) -> ()) -> URLSessionTask
//    static func makePostRequest<T: Codable> (path: EndPoint, body: Parameters, onCompletion: @escaping (Result<T, NetworkError>) -> ()) -> URLSessionTask
//}
//
//
//fileprivate enum APIRequestManager: APIRequestProtocol {
//    case getAPI(path: EndPoint, data: Parameters)
//    case postAPI(path: EndPoint, data: Parameters)
//    
//    static var baseURL: URL = URL(string: "https://jsonplaceholder.typicode.com/")!
//    
//    private var path: EndPoint {
//        switch self {
...
//        var request = URLRequest(url: Self.baseURL.appendingPathComponent(path.rawValue))
//        
...
//    fileprivate static func makeRequest<S: Codable>(session: URLSession, request: URLRequest, model: S.Type, onCompletion: @escaping(Result<S, NetworkError>) -> ()) -> URLSessionTask {
...
//    
//    /// Generic GET Request
//    fileprivate static func makeGetRequest<T: Codable> (path: EndPoint, queries: Parameters, onCompletion: @escaping(Result<T, NetworkError>) -> ()) -> URLSessionTask {
//        let session = URLSession.shared
//        let request: URLRequest = Self.getAPI(path: path, data: queries).asURLRequest()
//        
//        let task = makeRequest(session: session, request: request, model: T.self) { (result) in
//            onCompletion(result)
//        }
//        return task
//    }
...
//    fileprivate static func makePostRequest<T: Codable> (path: EndPoint, body: Parameters, onCompletion: @escaping (Result<T, NetworkError>) -> ()) -> URLSessionTask {
//        let session = URLSession.shared
